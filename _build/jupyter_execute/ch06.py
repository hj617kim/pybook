...
# :::{admonition} 주의  
# :class: caution  
# 파이썬을 포함해서 많은 대부분의 프로그래밍 언어에서 인덱싱은 0부터 시작한다. 따라서 첫째 문자를 확인하고자 할 때는 1이 아닌 0을 인덱스로 사용해야 한다.
# :::
...
# 시작인덱스의 값이 끝인덱스 값보다 같거나 작아야 제대로 추출한다. 그렇지 않으면 빈문자열이 추출된다.
...
# ## 문자열 관련 메서드
# 
# 문자열 자료형에만 사용하는 함수들이 있다. 이와같이 특정 자료형에만 사용하는 함수들을 메서드(method)라 부른다.  
# 
# 보다 자세한 설명은 여기서는 하지 않는다. 다만 특정 자료형을 잘 다루기 위해서는 어떤 경우에 어떤 메서드를 유용하게 활용할 수 있는지를 잘 파악해두는 것이 매우 중요하다는 점만 강조한다. 
# 

# ### 메서드 호출방법
# 
# 예를 들어, `banana` 문자열을 이용하여 `BANANA`처럼 모두 대문자로만 구성된 문자열을 생성하고자 한다면 `upper`라는 메서드를 아래와 같이 사용한다.
# 
# > `'banana'.upper()` 
# 
# 메서드는 일반적인 함수들과는 달리, 특정 자료형의 값이 먼저 언급된 다음에 호출된다. 

# ### 문자열 메서드 추가 예제
# 
# `upper()` 메서드 이외에 문자열과 관련된 메서드는 매우 많다. 여기서는 가장 많이 사용되는 메서드 몇 개를 소개하고자 한다. 
# 
# * `strip()`
# * `split()`
# * `replace()`
...
# * `strip()` 메서드는 문자열의 양 끝을 지정한 문자열 기준으로 삭제하는 방식으로 정리한다. 
...
# * `split()` 메서드는 지정된 부분문자열을 기준으로 문자열을 쪼개어 문자열들의 리스트로 반환한다. 리스트 자료형은 이후에 자세히 다룬다.  
# 
...
# 예를 들어, `strip()` 메서드를 먼저 실행한 다음에 그 결과에 `split()` 메서드를 실행하면 좀 더 산뜻한 결과를 얻을 수 있다. 
...
# :::{admonition} 주의  
# :class: caution  
# 아래와 같이 `split()` 메서드를 실행한 다음에 그 결과에 `strip()` 메서드를 실행하면 오류가 발생한다. 앞에서 언급한 것처럼 `split()` 메서드는 지정된 부분문자열을 기준으로 문자열을 쪼개어 문자열들의 리스트로 반환하고, 리스트에는 `strip()` 메서드가 없기 때문이다.
# 
# ```python
...
# * `replace()` 메서드는 하나의 문자열을 다른 문자열로 대체한다. 
#  
...
# * `caplitalize()` 메서드는 제일 첫 문자를 대문자로 변환시킨다.  
# 
...
# * `title()` 메서드는 각각의 단어의 첫 문자를 대문자로 변환시킨다.  
# 
...
# * `startswith()` 메서드는 문자열이 특정 문자열로 시작하는지 여부를 판단해준다.
...
# * `endswith()` 메서드는 문자열이 특정 문자열로 끝나는지 여부를 판단해준다.
...
# ## 불변 자료형
# 
# 파이썬의 문자열 자료형의 값들은 변경이 불가능하다. 앞서 `week_days`에 할당된 문자열에 다양한 메서드를 적용하여 새로운 문자열을 생성하였지만 `week_days`에 할당된 문자열 자체는 전혀 변하지 않았음을 아래와 같이 확인할 수 있다. 
